      ...
      while(pop_next(&next)){
        if(next){
          ...
          id = next->id;
                ...
                filter_old(f,{*(next->data)},var_a_pred->blocks[id]);
                next->remove();
        }
        next=NULL;
      }
    ...
    inline void operation(void)
    {
      ...
      do {
        ...
        if(next){
          id=next->id;
          block = getSinglePush();
                filter_old( f,{*(next->data)},block->data);
                next->remove();
                send(id,block);
                block=NULL;
        }
  ...
  public:
    ...
    inline void exec (Input_1 *in1, Input_2 *in2)
    {
      ...
      if(in1 && in2){
        ...
        id=in1->id;
        block = getSinglePush();
            krao_old(*(in1->data),*(in2->data),block->data);
  ...
  public:
    ...
    inline void exec (Input_1 *in1, Input_2 *in2)
    {
      ...
      if(in1 && in2){
        ...
        id=in1->id;
            block = getSinglePush();
            lift_old(f,{*(in1->data),*(in2->data)},block->data);
            in1->remove();in2->remove();
            send(id,block);
            block=NULL;
      }
