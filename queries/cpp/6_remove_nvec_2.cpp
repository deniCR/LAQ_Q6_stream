...
#include <dot.hpp>
#include <filter_old.hpp>
#include <fold_old.hpp>
#include <functions.hpp>
#include <krao_old.hpp>
#include <lift_old.hpp>
#include <matrix.hpp>
#include <types.hpp>
	...
	#include "../../papi_counters/papi_omp.h"
  	#define NUM_EVENTS 3
  	int my_events[NUM_EVENTS] = { PAPI_LD_INS, PAPI_SR_INS, PAPI_L2_DCM };
#endif
...
int main() {
  	...
  	for (engine::Size i = 0; i < var_lineitem_shipdate->nLabelBlocks; ++i)
  	{
		...
		#endif
		filter_old(filter_var_a,{*(var_lineitem_shipdate->labels[i])},var_a_pred->blocks[i]);
	  	#ifdef D_VTUNE
			vtd->end_task();
		#endif
