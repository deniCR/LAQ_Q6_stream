...
def main(argv):
  ...
  for num in queries_keys:
    ...
    h=20;w=20;
    event_results = [[[] for y in range(h)] for y in range(h)] 
    ...
    for i in range(15):
      ...
      for ops in range(2,17):
        event = output_line_list[ops].split();
        for ix in range(0,4):
          event_results[ix][ops-2].append( float(event[ix]))
    ...
    with open("perf_collect/results/{0}_time.csv".format(OUTPUT_FILE), "a") as csv_time:
      ...
      for ops in range(0,15):
        for ix in range(0,4):
          csv_time.write( "{0},".format(avg(event_results[ix][ops])))
        csv_time.write("\n");
      csv_time.write("\n");
